...
export default class ExpenseFab extends LightningElement {
    ...
    // Visit linking
    @track availableVisits   = [];   // [{ id, name, outletName, selected }]
    ...
    openModal() {
        ...
        // Load visits for today's date immediately
    ...
    async _loadVisitsForDate(dateStr) {
        ...
        try {
            ...
            // Preserve selected state if user already ticked some visits
    ...
    handleInput(event) {
        ...
        // Reload visits when date changes
    ...
    async handleSave() {
        ...
        const selectedVisits = this.selectedVisitIds;   // captured before reset

        // Capture form values BEFORE closeModal() resets formData
        const expenseData = {
        ...
        // Enqueue expense fields (offline-safe); visits will sync with the expense
        const result = enqueue({
            amount     : expenseData.amount,
            expenseType: expenseData.expenseType,
            expenseDate: expenseData.expenseDate,
            description: expenseData.description,
            visitIds   : expenseData.visitIds   // stored for offline sync
        });
    ...
    // ── Sync (expense fields + visitIds — files never queued) ──
