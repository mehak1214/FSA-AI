...
export default class ExpenseList extends LightningElement {
    ...
    @track _collapsedGroups = new Set(); // date keys that are collapsed
    ...
    // Collapse all date groups except the 3 most recent
    _initCollapsedGroups(expenses) {
        const keys = [...new Set(
            expenses
                .map(e => e.expenseDate || 'Unknown Date')
                .filter(Boolean)
        )].sort((a, b) => b.localeCompare(a));   // descending

        const collapsed = new Set();
        keys.forEach((key, idx) => {
            if (idx >= 3) collapsed.add(key);    // collapse everything beyond index 2
        });
        this._collapsedGroups = collapsed;
    }
    ...
    // Groups expenses by expenseDate, latest date first — used in "All" tab
    get groupedExpenses() {
        ...
        return sortedKeys.map(key => {
            ...
            return {
                key,
                dateLabel: key === 'Unknown Date' ? 'Unknown Date' : new Date(key + 'T00:00:00').toLocaleDateString('en-IN', {
                    weekday: 'short', day: '2-digit', month: 'short', year: 'numeric'
                }),
                expenses       : map.get(key),
                isCollapsed,
                headerClass    : 'expense-date-header' + (isCollapsed ? ' is-collapsed' : ''),
                bodyStyle      : isCollapsed ? 'display:none' : '',
                chevronClass   : 'date-chevron' + (isCollapsed ? ' chevron-collapsed' : '')
            };
        });
    }
    ...
    handleToggleGroup(event) {
        ...
        // LWC @track doesn't detect Set mutations — replace with a new Set
